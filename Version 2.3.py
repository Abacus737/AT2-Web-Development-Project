...
class User(db.Model, UserMixin):
    ...
    # Use a function reference for default timestamps
...
def edit_review(review_id):
    ...
    if request.method == 'POST':
        review_obj.film_title = request.form['film_title']
        review_obj.rating = int(request.form['rating'])
        review_obj.text = request.form['text']
        db.session.commit()
...
def comment(review_id):
    ...
    review_obj = Review.query.get_or_404(review_id)
    new_comment = Comment(text=text, review_id=review_obj.id, user_id=current_user.id)
